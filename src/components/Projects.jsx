...
const ProjectCard = ({ project, index }) => {
  ...
  const isWebsiteLink = project.title === 'BeamMP'
  return (
      ...
      <TiltCard className="group h-full">
        <article onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="h-full glass-effect rounded-3xl overflow-hidden border border-white/10 group-hover:border-primary/50 transition-colors duration-300 shadow-lg group-hover:shadow-2xl group-hover:shadow-primary/20">
          <div className="relative h-48 overflow-hidden" style={{ background: 'linear-gradient(135deg, #1a0a00 0%, #2d1000 50%, #1a0800 100%)' }}>
            {bannerSrc ? (<img src={bannerSrc} alt={project.title} className="w-full h-full object-contain p-4 transition-transform duration-500 group-hover:scale-105" />) : (<div className={`w-full h-full bg-gradient-to-br ${project.gradient}`} />)}
